if __name__ == "__main__":
    ...
    class Laptop(Computer):
        ...
        """
if __name__ == '__main__':
    # Создание экземпляра класса Computer
    pc = Computer("Dell", "OptiPlex", 16, 512)
    print(pc)
    # Компьютер Dell OptiPlex, ОЗУ: 16 ГБ, Память: 512 ГБ

    # Создание экземпляра класса Laptop
    laptop = Laptop("Apple", "MacBook Pro", 16, 1024, 13.3, 10)
    print(laptop)
    # Ноутбук Apple MacBook Pro, ОЗУ: 16 ГБ, Память: 1024 ГБ, Экран: 13.3" дюймов, Время работы: 10 ч.

    # Попытка увеличить ОЗУ ноутбука
    laptop.upgrade_ram(8)
    # ОЗУ увеличена до 24 ГБ.

    # Проверка уровня заряда батареи ноутбука
    laptop.check_battery()
    # Остаток заряда батареи: 10 часов.
